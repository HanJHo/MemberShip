        ...
        // 권한 페이지 접속권한
                .authorizeHttpRequests(
                        authorization -> authorization
                                .requestMatchers("/user/signup/**").permitAll()       // 로그인 페이지는 누구나 접속이 가능한 권한을 부여
...
//                                .anyRequest().authenticated()   // 그 이외에는 다 로그인해서 접속해
        ...
        // 로그아웃
                .logout(
                        logout -> logout
                                .logoutRequestMatcher(new AntPathRequestMatcher("/user/logout"))   // 로그아웃, a 태그라고 생각
                                                                        // <a href="/user/logout">잘가~</a>
                                .invalidateHttpSession(true)            // 세션 초기화
                                .logoutSuccessUrl("/")                  // localhost:8090 으로 간다.
                                                                        // dns 주소일 경우 www.naver.com 까지 로 간다.
                                                                        // 컨트롤러에서 만들어줄껄?
                )

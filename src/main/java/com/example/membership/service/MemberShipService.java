...
public class MemberShipService implements UserDetailsService {
    ...
    // 회원가입 : 컨트롤러에서 dto 를 입력받아 entity 로 변환하여 repository 의
    // save 를 이용해서 저장한다.
    // 반환값은 뭐로 할까? : dto 전체를 반환으로 하자
    // 그런데 만약에 회원이 가입이 되어있으면 어쩔꺼임?

    public MemberShipDTO saveMember(MemberShipDTO memberShipDTO){

        // 사용자가 이미 있는지 확인
        // 가입하려는 email 로 이미 사용자가 가입이 되어있는지 확인한다.
        MemberShip memberShip =
        memberShipRepository.findByEmail(memberShipDTO.getEmail());

        if(memberShip != null){     // 확인했더니 이미 가입이 되어있디면
    ...
    // 로그인  // UserDetailsService 를 구현해서 사용한다.
    @Override
    public UserDetails loadUserByUsername(String email) throws UsernameNotFoundException {
        // 사용자가 입력한 email 을 가지고 db 에서 검색한다.
        // 그러면 email 과 password 를 포함한 entity 를 받을 것이다.
        // 로그인은 기본적으로 email 과 passoword 를 찾거나
        // email 로 검색해서 가져온 데이터가 null 이 아니라면 그 안에 있는 password 를 가지고
        // 비교해서 맞다면 로그인한다.
        log.info("유저디테일서비스로 들어온 이메일 : " + email);   // 안들어왔다면 input 창으로 넣은 값이 도달을 못함
        MemberShip memberShip =
        this.memberShipRepository.findByEmail(email);
        // 이메일로 검색해서 가져온 값이 없다면, 그러니까 회원가입이 되어있지 않다면 예외 처리를 해주겠다.
        // try catch 문으로 다른화면으로 달리던 메시지를 가지고 로그인창으로 보내던 컨트롤러 창에서 알아서 해주겠지
        // 미래의 내가
        if (memberShip == null) {
